...
node -e "const v=process.versions.node.split('.').map(Number); if (v[0] < 22 || (v[0] === 22 && v[1] < 13)) { console.error('Node.js 22.13 oder neuer erforderlich, gefunden ' + process.version); process.exit(1); }"
