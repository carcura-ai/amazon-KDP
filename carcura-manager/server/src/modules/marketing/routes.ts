...
export default async function marketingRoutes(app: FastifyInstance) {
  ...
  app.get('/api/marketing/overview', { preHandler: app.requireAuth('marketing:read') }, async (req) => {
    ...
    const seo = hasSeo ? { clicks: se.clicks, impressions: se.impressions, position: se.position === null ? null : Math.round(se.position * 10) / 10, prev: { clicks: sep.clicks, impressions: sep.impressions }, queries: app.db.select({ name: seoDaily.dimensionValue, clicks: sql<number>`sum(${seoDaily.clicks})`, impressions: sql<number>`sum(${seoDaily.impressions})`, position: sql<number | null>`round(avg(${seoDaily.position}), 1)` }).from(seoDaily).where(and(eq(seoDaily.companyId, cid), eq(seoDaily.dimensionType, 'query'), gte(seoDaily.date, from), lt(seoDaily.date, toExcl))).groupBy(seoDaily.dimensionValue).orderBy(sql`2 desc`).limit(10).all() } : null;
