...
export function buildTools(app: FastifyInstance, companyId: string, opts: { personalData?: boolean } = {}) {
  const db = app.db;
  const personal = opts.personalData === true;
  /** Ohne Freigabe werden Kunden nur pseudonym (Kundennummer) an die KI übermittelt. */
  const pseudo = (row: { customerNumber?: string | null; name?: string | null; company?: string | null; phone?: string | null; email?: string | null; customer?: string | null }) =>
    personal ? row : { ...row, name: row.customerNumber ? `Kunde ${row.customerNumber}` : 'Kunde', company: null, phone: null, email: null, customer: row.customerNumber ? `Kunde ${row.customerNumber}` : row.customer ? 'Kunde' : null };
  ...
  const run = async (name: string, input: Record<string, unknown>): Promise<unknown> => {
    ...
    switch (name) {
      ...
      case 'get_open_invoices': return { items: db.select({ number: invoices.invoiceNumber, status: invoices.status, total: invoices.totalCents, paid: invoices.paidCents, dueDate: invoices.dueDate, customer: sql<string>`(select first_name || ' ' || last_name from customers c where c.id = ${invoices.customerId})`, customerNumber: sql<string>`(select customer_number from customers c where c.id = ${invoices.customerId})` }).from(invoices).where(and(eq(invoices.companyId, companyId), sql`${invoices.status} in ('open','sent','overdue')`)).orderBy(invoices.dueDate).limit(50).all().map((i) => ({ ...pseudo(i), number: i.number, status: i.status, dueDate: i.dueDate, open: money(i.total - i.paid), total: money(i.total) })) };
      ...
      case 'get_inactive_customers': { const cutoff = new Date(Date.now() - days * 86_400_000).toISOString(); return { days, customers: db.select({ customerNumber: customers.customerNumber, name: sql<string>`${customers.firstName} || ' ' || ${customers.lastName}`, company: customers.companyName, phone: customers.phone, email: customers.email, lastOrder: sql<string | null>`(select max(created_at) from orders o where o.customer_id = customers.id)` }).from(customers).where(and(eq(customers.companyId, companyId), eq(customers.isActive, true), sql`coalesce((select max(created_at) from orders o where o.customer_id = customers.id), ${customers.createdAt}) < ${cutoff}`)).limit(50).all().map(pseudo) }; }
...
- Kunden können pseudonymisiert als „Kunde KD-000123“ erscheinen; nenne dann die Kundennummer.`;

export interface ChatTurn { role: 'user' | 'assistant'; content: string }

export async function runAssistant(app: FastifyInstance, companyId: string, companyName: string, cfg: AssistantConfig, history: ChatTurn[], question: string, fetchFn?: typeof fetch, personalData = false): Promise<{ answer: string; toolsUsed: string[]; inputTokens: number; outputTokens: number; stopReason: string }> {
  const client = new Anthropic({ apiKey: cfg.apiKey, fetch: fetchFn, maxRetries: 2, timeout: 120_000 });
  const tools = buildTools(app, companyId, { personalData });
